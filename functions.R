...
# INLArerun_with_Retry <- function(Result, N_RERUN_retry=3){
#   retry <- as.integer(0)
#   result <- NULL
#   while(retry<=N_RERUN_retry){
#     result <- tryCatch({
#       inla.rerun(Result)}, 
#       error = function(e){
#         cat("Error in INLA.RERUN model fitting. Retrying...", retry+1, "\n", sep = "")
#         Sys.sleep(1)
#         NULL
#       })
#     if(!is.null(result)){break} 
#     retry <- retry + 1
#     if(retry > N_RERUN_retry){
#       message("Error in INLA.RERUN model fitting. Exceeded max number of retries \n")
#       result <- list(dic = list(dic = NULL))
#       break 
#     }
#   }
...
fit_model2 <- function(KMR, ClearType, SpatUnits = SUs, RespData = ZStats_Woody, CovsCD, SA1sPoly = SA1s, Explanatory = "All", Verbose = TRUE, NMod_TOLN = 0.005 , N_retry=3, N_rerun = 0, Initial_Tlimit = 1000, OutputDir = NULL) {
  ...
  # NMod_TOLN = tolerance for INLA model fitting (passed to INLA_with_Retry)
...
fit_model3 <- function(KMR, ClearType, SpatUnits = SUs, RespData = ZStats_Woody, CovsCD, SA1sPoly = SA1s, Explanatory = "All", Verbose = TRUE, NMod_TOLN = 0.005 , N_retry=3, N_rerun = 0, Initial_Tlimit = 1000, OutputDir = NULL) {
  # ## Same as fit_model but with additional control for Explanatory variables and Verbose, allow for debugging
  # KMR = text field for KMR - one of "CC", "CST", "DRP", "FW", "NC", "NT", "NS", "R", "SC"
  # ClearType = clearing type - one of 1 = agriculture, 2 = infrastructure, 3 = forestry
  # SpatUnits = spatial units as list of Spatvector objects (properties) for each KMR
...
refit_model <- function(KMR = "CC", ClearType = 1, N_rerun = 0 , NMod_TOLN = 0.005, ModelDir = "output/models/"){
  ...
  Model <- fit_model2(KMR, ClearType, SpatUnits = SpatUnits, RespData = RespData, CovsCD = CovsCD, SA1sPoly = SA1sPoly, 
