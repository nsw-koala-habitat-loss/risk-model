...
get_zonal2 <- function(ZonalLayer, Raster, Stat) {
  return(as_tibble(exact_extract(Raster, ZonalLayer, Stat, max_cells_in_memory = 4e+08 )))
}
...
# Wrapper to handle INLA.RERUN errors and retry----
INLArerun_with_Retry <- function(Result, N_RERUN_retry=3, Verbose = FALSE){
  retry <- as.integer(0)
  result <- Result
  for(i in 1:N_RERUN_retry){
    if(Verbose){cat("Attempt ", i, " to rerun INLA model...\n")}
    TEMP_result <- tryCatch({
      inla.rerun(result)}, 
      error = function(e){
        cat("Error in attempt ", i, ": ", conditionMessage(e))
        Sys.sleep(1)
        return(NULL)
      })
...
#' @param CovsCD covariates for each spatial unit as a list for each KMR
#' @param SA1sPoly SA1s spatial representation as a list of Spatvector objectd for each KMR
#' @param Direction direction of model selection - one of "Forward", "F", "Backward", "B", "Forward Complete", "FC", "Backward Complete", "BC"
#' @param NMod_TOLN tolerance for INLA model fitting (passed to INLA_with_Retry)
#' @param Verbose logical to print progress
#' @param N_rerun number of reruns for INLA model fitting (passed to INLArerun_with_Retry)
#' @param N_retry number of retries for INLA model fitting (passed to INLA_with_TimeLimit)
#' @param Initial_Tlimit initial time limit for INLA model fitting (passed to INLA_with_Retry)
#' @param OutputDir directory to save model output (optional)
Select_model <- function(KMR = "CC", ClearType = 1, SpatUnits = SUs_Ag, RespData = ZStats_Woody_Ag, CovsCD = ZStats_Covs_Ag, SA1sPoly = SA1s, Direction = "Forward Complete", NMod_TOLN = 0.005 , Verbose = FALSE, N_rerun = 0, N_retry=3, Initial_Tlimit = 1000, OutputDir = NULL) {
  tic("Total Time")
  ...
  if(Direction == "Forward Complete" | Direction == "FC"){
    ...
    stop("Invalid model selection method\n Please select one of 'Complete Forward', 'CF' or 'Complete Backward', 'CB'")
  }
...
# Function for model selection by using prior from the full model----
#' @description This is only for models that are having convergence issues. It uses the priors from the full model to help with convergence during model selection.
#' @param KMR text field for KMR - one of "CC", "CST", "DRP", "FW", "NC", "NT", "NS", "R", "SC"
#' @param ClearType clearing type - one of 1 = agriculture, 2 = infrastructure, 3 = forestry
#' @param SpatUnits spatial units as list of Spatvector objects (properties) for each KMR
#' @param RespData response data consisting listr of clearing and woody vegetation data for each KMR
...
#' @param Initial_Tlimit initial time limit for INLA model fitting (passed to INLA_with_Retry)
#' @param OutputDir directory to save model output (optional)
Select_model2 <- function(KMR = "CC", ClearType = 1, SpatUnits = SUs_Ag, RespData = ZStats_Woody_Ag, CovsCD = ZStats_Covs_Ag, SA1sPoly = SA1s, Direction = "Forward Complete", NMod_TOLN = 0.005 , Verbose = FALSE, N_rerun = 0, N_retry=3, Initial_Tlimit = 1000, OutputDir = NULL) {
  tic("Total Time")
  ...
  ## extract fixed effects estimates to use as priors
  Fixed_meanN <- setNames(c(FullResultN$summary.fixed[(2:nrow(FullResultN$summary.fixed)),1], 0), c(rownames(FullResultN$summary.fixed)[2:nrow(FullResultN$summary.fixed)], "default"))
  Fixed_precN <- setNames(1/((c(FullResultN$summary.fixed[(2:nrow(FullResultN$summary.fixed)),2], 0.001)^2)*10), c(rownames(FullResultN$summary.fixed)[2:nrow(FullResultN$summary.fixed)], "default"))
  ...
  if(Direction == "Forward Complete" | Direction == "FC"){
    ...
    while(length(left_explV_ls) > 0){
      ...
      for(x in 1: length(left_explV_ls)){
        ...
        ## extract fixed effects estimates to use as priors
        Prior_meanP <- Fixed_meanP[match(c(unlist(strsplit(explV, " + ", fixed=TRUE)), "default"), names(Fixed_meanP))]
        Prior_precP <- Fixed_precP[match(c(unlist(strsplit(explV, " + ", fixed=TRUE)), "default"), names(Fixed_precP))]
        Prior_meanN <- Fixed_meanN[match(c(unlist(strsplit(explV, " + ", fixed=TRUE)), "default"), names(Fixed_meanN))]
  ...
  ## extract fixed effects estimates to use as priors
  Prior_meanP <- Fixed_meanP[match(c(unlist(strsplit(paste(Sel_explV_ls, collapse=" + "), " + ", fixed=TRUE)), "default"), names(Fixed_meanP))]
  Prior_precP <- Fixed_precP[match(c(unlist(strsplit(paste(Sel_explV_ls, collapse=" + "), " + ", fixed=TRUE)), "default"), names(Fixed_precP))]
  Prior_meanN <- Fixed_meanN[match(c(unlist(strsplit(paste(Sel_explV_ls, collapse=" + "), " + ", fixed=TRUE)), "default"), names(Fixed_meanN))]
...
# function to refit the model from selected model to enable inla.posterior.sample ----
#' @description To include control.compute = list(dic = TRUE, config = TRUE)
#' @param KMR text field for KMR - one of "CC", "CST", "DRP", "FW", "NC", "NT", "NS", "R", "SC"
#' @param ClearType clearing type - one of 1 = agriculture, 2 = infrastructure, 3 = forestry
#' @param N_rerun number of reruns for INLA model fitting (passed to INLArerun_with_Retry)
#' @param NMod_TOLN tolerance for INLA model fitting (passed to INLA_with_Retry)
#' @param ModelDir directory to read and save model output 
refit_model <- function(KMR = "CC", ClearType = 1, N_rerun = 0 , NMod_TOLN = 0.005, ModelDir = "output/models/"){
  ...
  Model <- fit_model(KMR, ClearType, SpatUnits = SpatUnits, RespData = RespData, CovsCD = CovsCD, SA1sPoly = SA1sPoly, 
...
#' @param model the fitted model (an output from fit_model)
#' @param N number of samples to draw from the posterior
#' @param RandEff the name of the random effect
#' @param verbose logical to print progress
predict_model3 <- function(model, N = 1000, RandEff = "SA1ID", verbose = TRUE){
  ...
  if(verbose) {cat("inla.posterior.sample object size: \n PModel: ", format(object.size(PModel_samp), units = "Mb"), 
                   "\n NModel: ",format(object.size(NModel_samp), units = "Mb"))}

  NModel_Phi <- mean(sapply(NModel_samp, function(x)(x$hyperpar[[1]])))
  gc()
  ...
  if(verbose) {cat("\n\nFixed effects samples: \n PModel: ", format(object.size(PModel_samp_fixed), units = "Mb"), 
                   "\n NModel: ",format(object.size(NModel_samp_fixed), units = "Mb"))}
  ...
  if(verbose) {cat("Random effects samples: \n PModel: ", format(object.size(PModel_samp_rand_mt), units = "Mb"), 
                   "\n NModel: ",format(object.size(NModel_samp_rand_mt), units = "Mb"))}
  gc()
  ...
  if(verbose) {cat("Fixed effects prediction matrix size: \n PModel: ", format(object.size(PredP_samp_fixed_mt), units = "Mb"), 
                   "\n NModel: ",format(object.size(PredN_samp_fixed_mt), units = "Mb"), "\n\n")}
  ...
  PredAll <- PredP * PredN
  if(verbose) {cat("Predictions matrix size: \n PModel: ", format(object.size(PredP), units = "Mb"), 
                   "\n NModel: ",format(object.size(PredN), units = "Mb"), "\n\n")}
  rm(PredP_samp_fixed_mt , PModel_samp_rand_mt, PredN_samp_fixed_mt , NModel_samp_rand_mt)
  
  if(verbose) {cat("\n\n\nSpatialising predictions...\n")}
  Layer <- SpatUnits[[KMR]] %>% 
    bind_cols(Woody_Clrtype = as.vector(R), Woody = as.vector(Response$N)) %>% 
    dplyr::filter(Woody > 0) %>% 
    mutate(ActualProp =  ifelse(Woody > 0, Woody_Clrtype / Woody, NA))
  PredictionsP <- rowMeans(PredP)
  PredictionsN <- rowMeans(PredN)
  PredictionsAll <- rowMeans(PredAll)
  
  # Check if all are the same length
  if(length(PredictionsP) != length(PredictionsN) || length(PredictionsP) != length(PredictionsAll) || length(PredictionsN) != nrow(Layer)) {
    stop("Predictions lengths do not match!")
  }
  Layer <- bind_cols(Layer, PredP = PredictionsP, PredN = PredictionsN, PredAll = PredictionsAll)
  if(verbose) {cat("Spatialised predictions: \n\n")
    print(head(Layer))}

  return(list(Layer = Layer, PModel = PModel, NModel = NModel, NModel_Phi = NModel_Phi))
}
...
#' @param ZStats_Woody Output from section "Select all covariates for each clearing type" *(clearing type data filtering above)
#' @param KMR_name_df A dataframe consist of KMR full name and KMR abbreviation
#' @return A dataframe consist of KHAB, SUID and KMR
Prep_Khab <- function(ZStats_Woody, KMR_name_df){
  Khab_data <- imap(ZStats_Woody, function(ZStat_Woody, KMR_a) {ZStat_Woody %>% mutate(KMR_a = KMR_a)}) %>% 
    do.call(rbind, .) %>% 
...
Get_cov_coeff_long <- function(ClearType, OUTPUT_DIR) {
  ...
  Model <- fit_model(KMR = kmr, ClearType = ClearType, SpatUnits = SUs, RespData = ZStats_Woody, CovsCD = ZStats_Covs, 
...
#' @param FILL Variable name to be plotted
#' @param LEGEND_Title Text for lebeling the legend
#' @param Inset_BL Dataframe with x,y of the Bottom left corner of the inset map
#' @param Inset_dim Dimension of the inset map
#' @param URB_PT_SUB1 list of urban points label for sub map 1 (get exact name from NSW_urb_pt$UCL_NAME16)
#' @param URB_PT_SUB2 list of urban points label for sub map 2 (get exact name from NSW_urb_pt$UCL_NAME16)
#' @param URB_PT_SUB3 list of urban points label for sub map 3 (get exact name from NSW_urb_pt$UCL_NAME16)
#' @param FilenamePath_PNG Directory to save the plot as a PNG file (optional) {set to NULL to not save}
#' @param PNG_width Width of the PNG file
#' @param PNG_height Height of the PNG file
#' @param PNG_dpi DPI of the PNG file
PLOTMAP_risk_with_3Insets <- function(DATA, FILL, LEGEND_Title = "Koala habitat\nloss risk", 
  ...
  Map_main <- ggplot() +
    ...
    geom_rect(aes(xmin = Inset_BL[1,1], xmax = Inset_BL[1,1]+Inset_dim, ymin = Inset_BL[1,2], ymax = Inset_BL[1,2]+Inset_dim), fill = "transparent", colour = "black", linewidth = 0.6) +
    geom_rect(aes(xmin = Inset_BL[2,1], xmax = Inset_BL[2,1]+Inset_dim, ymin = Inset_BL[2,2], ymax = Inset_BL[2,2]+Inset_dim) , fill = "transparent", colour = "black", linewidth = 0.6) + 
    geom_rect(aes(xmin = Inset_BL[3,1], xmax = Inset_BL[3,1]+Inset_dim, ymin = Inset_BL[3,2], ymax = Inset_BL[3,2] + Inset_dim), fill = "transparent", colour = "black", linewidth = 0.6) +
    
    geom_text(data = data.frame(x = Inset_BL[1,1] + Inset_dim/2, y = Inset_BL[1,2] + Inset_dim/2), aes(x = x, y = y, label = "I"), size = 4, fontface = "bold", color = "black", bg.color = "grey90", bg.r = 0.05)+
    ...
    theme(axis.title.x = element_blank(), axis.title.y = element_blank())+
    theme(panel.background = element_rect(fill = "#C7E6F5", color = NA), plot.background  = element_rect(fill = "white", color = NA))+
    coord_sf(xlim = st_bbox(KMR_shp)[c(1,3)] + c(-15000, 15000), ylim = st_bbox(KMR_shp)[c(2,4)] + c(-5000, 5000), expand = FALSE)
  
  Map_sub1 <- ggplot() +
    
    geom_sf(data = STE, fill = "grey70", color = "black", lwd = 0.2)+
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
    theme(axis.title.x = element_blank(), axis.title.y = element_blank())+
    # theme_void() + theme(plot.background  = element_rect(fill = "#C7E6F5", color = NA))+
    coord_sf(xlim = c(Inset_BL[1,1],  Inset_BL[1,1]+Inset_dim ), ylim =  c( Inset_BL[1,2], Inset_BL[1,2]+Inset_dim), expand = FALSE)
  ...
  Map_sub2 <- ggplot() +
    ...
    theme(axis.title.x = element_blank(), axis.title.y = element_blank())+
    # theme_void() + theme(plot.background  = element_rect(fill = "#C7E6F5", color = NA))+
    coord_sf(xlim = c(Inset_BL[2,1],  Inset_BL[2,1]+Inset_dim ), ylim =  c( Inset_BL[2,2], Inset_BL[2,2]+Inset_dim), expand = FALSE)
  ...
  Map_sub3 <- ggplot() +
    ...
    theme(axis.title.x = element_blank(), axis.title.y = element_blank())+
    # theme_void() + theme(plot.background  = element_rect(fill = "#C7E6F5", color = NA))+
    coord_sf(xlim = c(Inset_BL[3,1],  Inset_BL[3,1]+Inset_dim ), ylim =  c( Inset_BL[3,2], Inset_BL[3,2]+Inset_dim), expand = FALSE)
  ...
  ggsave(FilenamePath_PNG, FINAL_map, width = 21, height = 23, units = "cm", dpi = PNG_dpi)

  return(FINAL_map)
}

# Function to plot Maps with Insets deforestation risk / Koala habitat loss ----
PLOTMAP_risk_with_6Insets <- function(DATA_ALL, FILL_ALL,
                                     DATA_Agri, FILL_Agri, DATA_Fort, FILL_Fort, DATA_Infr, FILL_Infr,
  ...
  Map_main <- ggplot() +
    ...
    geom_sf(data = DATA_ALL, aes(fill = {{FILL_ALL}}), color = NA)+
    scale_fill_gradientn(colours = COLOUR, name = LEGEND_Title)+
    ...
    geom_rect(aes(xmin = Inset_BL[2,1], xmax = Inset_BL[2,1]+100000, ymin = Inset_BL[2,2], ymax = Inset_BL[2,2]+100000), fill = "transparent", colour = "black", linewidth = 0.6) + 
    geom_rect(aes(xmin = Inset_BL[3,1], xmax = Inset_BL[3,1]+100000, ymin = Inset_BL[3,2], ymax = Inset_BL[3,2] + 100000), fill = "transparent", colour = "black", linewidth = 0.6) +
    
    geom_text(data = data.frame(x = Inset_BL[1,1] + 100000/2, y = Inset_BL[1,2] + 100000/2), aes(x = x, y = y, label = "I"), size = 4, fontface = "bold", color = "black", bg.color = "grey90", bg.r = 0.05)+
    geom_text(data = data.frame(x = Inset_BL[2,1] + 100000/2, y = Inset_BL[2,2] + 100000/2), aes(x = x, y = y, label = "II"), size = 4, fontface = "bold", color = "black", bg.color = "grey90", bg.r = 0.05)+
    geom_text(data = data.frame(x = Inset_BL[3,1] + 100000/2, y = Inset_BL[3,2] + 100000/2), aes(x = x, y = y, label = "III"), size = 4, fontface = "bold", color = "black", bg.color = "grey90", bg.r = 0.05)+
    ...
    theme(axis.title.x = element_blank(), axis.title.y = element_blank())+
    theme(panel.background = element_rect(fill = "#C7E6F5", color = NA), plot.background  = element_rect(fill = "white", color = NA))+
    coord_sf(xlim = st_bbox(KMR_shp)[c(1,3)] + c(-50000, 75000), ylim = st_bbox(KMR_shp)[c(2,4)] + c(-5000, 5000), expand = FALSE)
  ...
  Map_sub2A <- ggplot() +
    ...
    geom_sf(data = DATA_Agri, aes(fill = {{FILL_Agri}}), color = NA)+
    scale_fill_gradientn(colours = COLOUR, name = LEGEND_Title)+
    geom_sf(data = KMR_shp, fill = NA, color = "grey20", lwd = 0.2, linetype= "dotdash")+
    ...
    labs(tag = "  I", subtitle = "Agriculture") + 
    theme(plot.tag.position = "topleft", plot.tag.location = "panel") +
  ...
  Map_sub2B <- ggplot() +
    ...
    geom_sf(data = DATA_Fort, aes(fill = {{FILL_Fort}}), color = NA)+
    scale_fill_gradientn(colours = COLOUR, name = LEGEND_Title)+
    geom_sf(data = KMR_shp, fill = NA, color = "grey20", lwd = 0.2, linetype= "dotdash")+
  ...
  Map_sub2C <- ggplot() +
    ...
    geom_sf(data = DATA_Infr, aes(fill = {{FILL_Infr}}), color = NA)+
    scale_fill_gradientn(colours = COLOUR, name = LEGEND_Title)+
    geom_sf(data = KMR_shp, fill = NA, color = "grey20", lwd = 0.2, linetype= "dotdash")+
    ...
    labs(tag = " III", subtitle = "Infrastructure") + 
    theme(plot.tag.position = "topleft", plot.tag.location = "panel") +
  ...
  row1_label <- wrap_elements(grid::textGrob(
    "B. Combined deforestation risk",
    x = unit(0.02, "npc"),
    gp = gpar(fontsize = 12),
    just = c("left", "centre")
  ))

  row2_label <- wrap_elements(grid::textGrob(
      "C. Deforestation risk by drivers",
      x = unit(0.02, "npc"),
      gp = gpar(fontsize = 12),
      just = c("left", "centre")
    ))

  Map_IN1 <- Map_sub1A | Map_sub1B | Map_sub1C
  Map_IN2 <- Map_sub2A | Map_sub2B | Map_sub2C

  FINAL_map <- Map_main / row1_label / Map_IN1 / row2_label / Map_IN2 +
    plot_layout(heights = c(2.4, 0.072, 1, 0.072, 1))

  ggsave(FilenamePath_PNG, FINAL_map, width = 18.5, height = 28.5, units = "cm", dpi = PNG_dpi)
...
fit_model_original <- function(KMR, ClearType, SpatUnits, RespData, CovsCD, SA1sPoly) {
...
#' @param Inset_BL Dataframe with x,y of the Bottom left corner of the inset map
#' @param Inset_dim Dimension of the inset map
#' @param URB_PT_Main list of urban points label for main map (get exact name from NSW_urb_pt$UCL_NAME16)
#' @param URB_PT_SUB1 list of urban points label for sub map 1 (get exact name from NSW_urb_pt$UCL_NAME16)
#' @param URB_PT_SUB2 list of urban points label for sub map 2 (get exact name from NSW_urb_pt$UCL_NAME16)
#' @param URB_PT_SUB3 list of urban points label for sub map 3 (get exact name from NSW_urb_pt$UCL_NAME16)
#' @param FilenamePath_PNG Directory to save the plot as a PNG file (optional) {set to NULL to not save}
